...
class TopicManager:
    def __init__(self):
        self.bots = itertools.cycle(
            [Bot(token=alert_bot, default=DefaultBotProperties(parse_mode='HTML', link_preview_is_disabled=True), session=AiohttpSession(proxy=_TG_PROXY_URL) if _TG_PROXY_URL else None) for
             alert_bot in config.alert_bots])
    ...
    async def edit_topic(self, name: str | bool = None, general: bool = False, thread_id: int = None,
                         emoji_id: str = None):
        if general:
    ...
    async def edit_topic_name(self, thread_id: int = None, name: str | bool = None, general: bool = False):
        if general:
            await next(self.bots).edit_general_forum_topic(self.bot_group, name)
        else:
            if not thread_id:
                raise Exception('thread_id not specified')
            try:
                await next(self.bots).edit_forum_topic(chat_id=self.bot_group, message_thread_id=thread_id, name=name)
            except TelegramAPIError:
                ...
