...
async def input_message(message: Message, state: FSMContext, album: list[Message] = False):
    storage = await state.get_data()
    menu = storage['menu']
...
async def input_button_name(message: Message, state: FSMContext):
    storage = await state.get_data()
    menu: Message = storage['menu']
...
async def view_alert_data(update: Union[Message, CallbackQuery], state: FSMContext):
    storage = await state.get_data()
    menu: Message = storage['menu']
    await remove_preload(state)
